#ifndef PD_MOTION_PLANNER_PLUGIN_HPP
#define PD_MOTION_PLANNER_PLUGIN_HPP
...
{
class PDMotionPlannerPlugin : public nav2_core::Controller
{
public:
    PDMotionPlannerPlugin() = default;
    ~PDMotionPlannerPlugin() = default;
...
private:
    ...
    // pose is transformed on demand in getNextPose(), exactly as the
    // reference Python implementation does.
    nav_msgs::msg::Path global_plan_;


    // Transforms a single pose (expressed in the plan's "map" frame) into
    // the robot's "base_footprint" frame using transform_tf, i.e. the
    // direct C++ equivalent of the reference Python's
    // transform_to_matrix() + get_pose_matrix() + pose_from_matrix() chain.
    ...
    // the farthest one that is still more than step_size_ away from the
    // robot (the robot sits at the origin once expressed in its own frame).
    geometry_msgs::msg::Pose getNextPose(const tf2::Transform & transform_tf);
};
}  // namespace robot_motion

#endif // PD_MOTION_PLANNER_PLUGIN_HPP
