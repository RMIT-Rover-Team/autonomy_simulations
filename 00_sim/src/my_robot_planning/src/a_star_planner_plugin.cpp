...
#include "my_robot_planning/a_star_planner_plugin.hpp"
...
// Lifecycle methods
void AStarPlannerPlugin::configure(
    ...
    std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
    ...
    if(!smooth_cli_->wait_for_action_server(std::chrono::seconds(3))){
        RCLCPP_ERROR(node_->get_logger(), "path smoothing not available");
    }
}

void AStarPlannerPlugin::cleanup()
{
    RCLCPP_INFO(node_->get_logger(), "Cleaning up plugin: %s of type AStarPlannerPlugin", name_.c_str());
}
void AStarPlannerPlugin::activate()
{
    RCLCPP_INFO(node_->get_logger(), "Activating plugin: %s of type AStarPlannerPlugin", name_.c_str());
}
void AStarPlannerPlugin::deactivate()
{
    RCLCPP_INFO(node_->get_logger(), "Deactivating plugin: %s of type AStarPlannerPlugin", name_.c_str());
}

// Main path planning logic method
nav_msgs::msg::Path AStarPlannerPlugin::createPlan(
    ...
    std::function<bool()> cancel_checker)
{
    ...
    while (!pending_nodes.empty() && rclcpp::ok()) {
        ...
        // its cost, heuristic, and a back-pointer to this node.
...
double AStarPlannerPlugin::manhattanDistance(const GraphNode & node1, const GraphNode & node2)
{
    return std::abs(node2.x - node1.x) + std::abs(node2.y - node1.y);
}

bool AStarPlannerPlugin::poseOnMap(const GraphNode & node)
{
    return node.x < static_cast<int>(costmap_->getSizeInCellsX()) && node.x >= 0 &&
        node.y < static_cast<int>(costmap_->getSizeInCellsY()) && node.y >= 0;
}

bool AStarPlannerPlugin::isNavigable(const GraphNode & node, int & cost)
{
    cost = costmap_->getCost(node.x, node.y);
    return cost < 252;
}

GraphNode AStarPlannerPlugin::worldToGrid(const geometry_msgs::msg::Pose & pose)
{
...
geometry_msgs::msg::Pose AStarPlannerPlugin::gridToWorld(const GraphNode & node)
{
...
PLUGINLIB_EXPORT_CLASS(robot_planning::AStarPlannerPlugin, nav2_core::GlobalPlanner)